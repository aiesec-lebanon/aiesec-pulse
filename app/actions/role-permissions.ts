...
export async function setRolePermission(
  ...
  // The anti-lockout guarantee, enforced on the server. The read-only
  // rendering of these two rows is a courtesy; this is the check.
  if (isLockedFullAccess(roleKey)) {
    return {
      ok: false,
      error: `${ROLE_NAMES[roleKey]} holds every permission and cannot be edited.`,
    };
  }
